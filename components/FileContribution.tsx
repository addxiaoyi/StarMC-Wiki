  ...
  return (
    <div className="p-6 bg-slate-50 rounded-2xl border border-slate-100">
      <div className="flex items-center gap-3 mb-4">
        ...
        <div>
          <h3 className="text-sm font-bold text-slate-900">贡献文档</h3>
          <p className="text-xs text-slate-500">拖拽 MD 文件提交审核</p>
        </div>
      </div>
      ...
      <div 
        ...
        className={`
          ...
          ${dragActive ? 'border-slate-900 bg-slate-100' : 'border-slate-200 bg-white'}
      ...
      >
        ...
        {status === 'idle' && (
          <>
            ...
            <p className="text-xs font-medium text-slate-600">点击或拖拽文件到此处</p>
            <p className="text-[10px] text-slate-400 mt-1">支持 Markdown (.md) 格式</p>
          </>
        )}

        {status === 'uploading' && (
          <div className="flex flex-col items-center">
            <Loader2 size={24} className="text-slate-900 animate-spin mb-3" />
        ...
        {status === 'success' && (
          <div className="flex flex-col items-center animate-in fade-in slide-in-from-bottom-2 duration-300">
            ...
            <p className="text-xs font-bold text-emerald-900">{message}</p>
            <button 
              onClick={(e) => { e.stopPropagation(); reset(); }}
              className="mt-4 text-[10px] font-bold text-emerald-700 hover:text-emerald-900"
            >
              继续上传
            </button>
          </div>
        ...
        {status === 'error' && (
          <div className="flex flex-col items-center animate-in fade-in slide-in-from-bottom-2 duration-300">
            ...
            <p className="text-xs font-bold text-rose-900">{message}</p>
            <button 
              onClick={(e) => { e.stopPropagation(); reset(); }}
              className="mt-4 text-[10px] font-bold text-rose-700 hover:text-rose-900 flex items-center gap-1"
            >
              <X size={10} /> 重试
            </button>
          </div>
