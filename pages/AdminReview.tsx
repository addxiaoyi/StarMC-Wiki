  ...
  if (!isAuthenticated) {
    return (
      <div className="min-h-[80vh] flex items-center justify-center px-6">
        <div className="max-w-md w-full bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-2xl shadow-slate-200/50">
          <div className="flex flex-col items-center mb-10">
            <div className="w-16 h-16 bg-slate-900 text-white rounded-2xl flex items-center justify-center shadow-xl shadow-slate-200 mb-6">
              <ShieldCheck size={32} />
            </div>
            <h1 className="text-2xl font-black text-slate-900 tracking-tight text-center">管理员登录</h1>
          ...
          <form onSubmit={handleLogin} className="space-y-6">
            <div className="space-y-2">
              ...
              <div className="relative group">
                ...
                <input 
                  ...
                  className="w-full pl-12 pr-4 py-3.5 bg-slate-50 border border-slate-100 rounded-2xl outline-none focus:bg-white focus:border-slate-900 transition-all font-medium text-slate-900"
            ...
            {error && (
              <div className="flex items-center gap-2 text-rose-500 bg-rose-50 px-4 py-3 rounded-xl border border-rose-100 text-xs font-bold animate-in fade-in slide-in-from-top-1 duration-200">
                <AlertCircle size={14} />
                {error}
              </div>
            )}
            ...
            <button 
              type="submit"
              className="w-full py-4 bg-slate-900 text-white font-bold rounded-2xl hover:bg-slate-800 hover:shadow-xl hover:shadow-slate-200 transition-all flex items-center justify-center gap-2"
            >
              <LogIn size={18} />
              验证并进入
            </button>
    ...
    <div className="max-w-5xl mx-auto px-6 py-12">
      <div className="flex items-center justify-between mb-12">
        ...
        {loading ? (
          ...
          <div className="py-32 bg-slate-50 rounded-[2rem] border-2 border-dashed border-slate-200 flex flex-col items-center justify-center text-center px-6">
            <div className="w-16 h-16 bg-white rounded-2xl shadow-sm flex items-center justify-center mb-6">
              <Clock size={32} className="text-slate-200" />
            </div>
            <h3 className="text-xl font-bold text-slate-900 mb-2">暂无待处理申请</h3>
          ...
          <div className="grid gap-6">
            {submissions.map((s) => (
              <div key={s.id} className={`
                ...
                ${s.status === 'pending' ? 'bg-white border-slate-200 shadow-xl shadow-slate-100' : 'bg-slate-50 border-transparent opacity-60'}
              `}>
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                  <div className="flex items-start gap-4">
                    <div className={`p-3 rounded-2xl ${s.status === 'pending' ? 'bg-slate-100 text-slate-600' : 'bg-slate-200 text-slate-400'}`}>
                      <FileText size={24} />
                    </div>
                    <div>
                      <h3 className="text-lg font-bold text-slate-900 flex items-center gap-3">
                        {s.filename}
                        {s.status === 'approved' && <span className="px-2 py-0.5 bg-emerald-100 text-emerald-600 text-[10px] uppercase tracking-widest rounded-full">已通过</span>}
                        {s.status === 'rejected' && <span className="px-2 py-0.5 bg-rose-100 text-rose-600 text-[10px] uppercase tracking-widest rounded-full">已拒绝</span>}
                      </h3>
                    ...
                    {s.status === 'pending' ? (
                      <>
                        <button 
                          ...
                          className="flex-1 md:flex-none px-6 py-2.5 bg-emerald-500 text-white font-bold rounded-xl hover:bg-emerald-600 hover:shadow-lg hover:shadow-emerald-200 transition-all flex items-center justify-center gap-2"
                        >
                          <Check size={18} /> 通过
                        </button>
                        <button 
                          onClick={() => handleAction(s.id, 'rejected')}
                          className="flex-1 md:flex-none px-6 py-2.5 bg-white border border-slate-200 text-slate-600 font-bold rounded-xl hover:bg-rose-50 hover:text-rose-600 hover:border-rose-200 transition-all flex items-center justify-center gap-2"
                        >
                          <X size={18} /> 拒绝
                        </button>
                      </>
                ...
                <div className="mt-6 p-4 bg-slate-900 rounded-2xl overflow-hidden">
                  ...
                  <pre className="text-xs text-slate-300 font-mono overflow-x-auto max-h-40 whitespace-pre-wrap p-2">
                    {s.content}
                  </pre>
                </div>
